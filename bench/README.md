...
> **Availability:** This override is a test and benchmark facility. It is compiled in only when the project is configured with `ALCP_ENABLE_TESTS=ON`, which every benchmark build is. Release and RTE packages do not read the variable at all and silently ignore it.

> **Note:** This can only **downgrade** the kernel level — setting a higher level on lower hardware has no effect.
> For example, setting `ZEN4` on a Zen3 CPU will still use Zen3 kernels. In builds where the override is compiled in, an invalid value will cause the process to exit with an error.
